...
def _query_int(query: dict, nome: str, padrao: int) -> int:
    valor = query.get(nome, [str(padrao)])[0]
    try:
        return int(valor)
    except ValueError:
        return padrao
...
def rota_gasto_mensal(user_id, query):
    ...
    df = database.get_purchases_df(user_id)
    return analytics.gasto_mensal(df, meses=meses)
...
def rota_por_produto(user_id, query):
    ...
    df = database.get_purchase_items_df(user_id)
...
def rota_dashboard(user_id, query):
    ...
    purchases_df = database.get_purchases_df(user_id)
    items_df = database.get_purchase_items_df(user_id)

    gasto_mensal = analytics.gasto_mensal(purchases_df, meses=meses)
    previsao = analytics.previsao_gastos(purchases_df, meses_futuros=meses_futuros)
    por_supermercado = analytics.gasto_por_supermercado(purchases_df)
...
class AnalyticsRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            self._send_json(500, {"erro": f"Erro interno: {str(e)}"})
