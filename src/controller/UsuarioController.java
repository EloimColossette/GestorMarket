...
public class UsuarioController implements HttpHandler {
    ...
    private void handleGet(HttpExchange exchange) throws Exception {
        logger.info("Recebendo Requisicao GET /usuarios");
        ...
        ApiResponse response = new ApiResponse(true, "Usuarios listados com sucesso", responseDtos);
        sendResponse(exchange, mapper.writeValueAsString(response), 200);
    }

    private void handlePost(HttpExchange exchange) throws Exception {
        logger.info("Criando requisição POST /usuarios");

        String body      = lerBody(exchange);
        UserRequest user = mapper.readValue(body, UserRequest.class);
        userService.criarUsuario(user);

        ApiResponse response = new ApiResponse(true, "Usuario criado com sucesso", null);
    ...
    private void handlePut(HttpExchange exchange) throws Exception {
        logger.info("Recebendo a requisição PUT /usuarios");
        ...
        ApiResponse response = new ApiResponse(true, "Usuario atualizado com sucesso", null);
        sendResponse(exchange, mapper.writeValueAsString(response), 200);
    }

    private void handlePatch(HttpExchange exchange) throws Exception {
        logger.info("Recebendo requisição PATCH /usuarios");

        int id           = getIdFromPath(exchange.getRequestURI().getPath());
        UserRequest user = mapper.readValue(lerBody(exchange), UserRequest.class);
        userService.atualizarParcialmenteUsuario(id, user);

        ApiResponse response = new ApiResponse(true, "Usuario atualizado parcialmente com sucesso", null);
        sendResponse(exchange, mapper.writeValueAsString(response), 200);
    }
    ...
    private void handleDelete(HttpExchange exchange) throws Exception {
        ...
        int id = getIdFromPath(exchange.getRequestURI().getPath());
        if (id <= 0) throw new ApiException("ID inválido", 400);

        userService.excluirUsuario(id);

        ApiResponse response = new ApiResponse(true, "Usuario deletado com sucesso", null);
        sendResponse(exchange, mapper.writeValueAsString(response), 200);
    }
